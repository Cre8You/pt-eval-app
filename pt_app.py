...
JOINT_CONFIG = {
    ...
    "股関節": {
        "rom": {"屈曲": 125, "伸展": 15, "外転": 45, "内転": 20, "外旋": 45, "内旋": 45, "SLR": 70}, # 追加
    ...
    "膝関節": {
        "rom": {"屈曲": 130, "伸展": 0, "足関節背屈": 20, "足関節底屈": 45, "SLR": 70}, # 追加
    ...
    "足関節": {
        "rom": {"背屈": 20, "底屈": 45, "膝関節屈曲": 130, "膝関節伸展": 0, "SLR": 70}, # 追加
...
for item, ref in JOINT_CONFIG[joint]["rom"].items():
    ...
    # SLRは腰部であっても神経学的評価として左右別で行うため除外
...
【条件】
今回は「計画書の更新」です。以下の【５項目】のみを出力してください。挨拶や前置きは不要です。強調には「【】」を使用し、アスタリスクは使わないこと。
・短期目標（100文字以内。変化の経過を踏まえて記載）
・長期目標（50文字以内。変化の経過を踏まえて記載）
・治療方針（120文字以内。変化の経過とPT考察を踏まえて記載）
・参加制限に対する具体的な対応方針（200文字以内、簡潔な「です・ます調」。文脈に合わせて適宜【改行】を入れ、読みやすく整理すること）
・機能障害に対する具体的な対応方針（200文字以内、簡潔な「です・ます調」。文脈に合わせて適宜【改行】を入れ、読みやすく整理すること）
"""
        else:
            prompt = f"""
