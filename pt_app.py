...
with st.sidebar:
    ...
    st.divider()
    st.header("🔄 計画書更新（再評価）")
    patient_change = st.text_area(
        "計画書更新用カルテ内容（任意）",
        placeholder=(
            "毎月の計画書更新時に、前回以降のカルテ内容を貼り付けてください。"
            "入力がある場合は計画書更新モードになります。"
        ),
        height=120,
    )
...
laxity_area, _ = st.columns([3, 2], gap="small")
with laxity_area:
    st.markdown("#### 左右評価項目")
    st.caption("該当する側にチェックしてください。片側につき0.5点です。")
    description_header, right_header, left_header = st.columns([5, 1, 1], gap="small")
    ...
    for item, criterion in LAXITY_BILATERAL_TESTS.items():
        ...
        description_column, right_column, left_column = st.columns([5, 1, 1], gap="small")
    ...
    for item, criterion in LAXITY_SINGLE_TESTS.items():
        item_key = LAXITY_SINGLE_KEYS[item]
        description_column, check_column = st.columns([5, 2], gap="small")
        with description_column:
            st.markdown(f"**{item}**：{criterion}")
        with check_column:
...
- **計画書更新用カルテ内容**: {summarize_free_text(patient_change)}
"""
    )
...
"""
        ...
        if patient_change:
            prompt = f"""
あなたは19年の経験を持つベテラン理学療法士です。以下の評価データと【計画書更新用カルテ内容】をもとに、指定された【条件】を厳格に守って文章を作成してください。
{common_data}
【計画書更新用カルテ内容】
{patient_change}
【条件】
...
・【優先的問題点】
  現在の評価結果と計画書更新用カルテ内容を統合し、疼痛、機能障害、ADL・参加制限、治療上の課題を総合して、優先順位の高い順に必ず次の形式で3項目を記載してください。
  1. 〇〇
  2. 〇〇
  3. 〇〇
  同じ問題の言い換えによる重複を避け、各項目は計画書へ転記しやすい簡潔な表現にしてください。入力情報から判断できない内容や新しい診断名を推測で追加しないでください。3項目を抽出できる情報がない場合も番号は省略せず、「情報不足：判断できる記載なし」など不足が分かる表現にしてください。
・【短期目標】（100文字以内。変化の経過を踏まえて記載）
・【長期目標】（50文字以内。変化の経過を踏まえて記載）
・【治療方針】（120文字以内。変化の経過とPT考察を踏まえて記載）
・【実施プログラム】
  計画書更新用カルテ内容に実際に記載されている実施内容だけを、身体部位とプログラムの種類ごとに整理して箇条書きにしてください。出力は原則5行、最大5行とし、情報が少ない場合は無理に5行へ水増ししないでください。
  個別の運動名や治療手技を細かく羅列せず、同じ身体部位・同じ目的の内容は1行へ統合し、基本的に「部位＋プログラムの種類」で表現してください。個別種目名は分類に必要な場合だけ使用してください。
  分類例は「頸肩部・肩甲帯のストレッチ」「胸郭・体幹部のストレッチ」「腰部・骨盤周囲のストレッチ」「股関節周囲筋の筋力エクササイズ」「膝関節周囲筋の筋力エクササイズ」「足関節・足部の可動域練習」「上肢・手指の機能練習」「立ち上がり・歩行・階段などの動作練習」「バランス練習」「自主トレーニング・生活指導」です。これは分類の参考であり、カルテに記載のない項目を追加してはいけません。
  5種類を超える場合は近い内容を統合し、複数回実施、現在の主症状や優先的問題点との関連、継続実施、動作・ADL改善との関連、自主トレーニングとしての指導の順に優先度を判断して5行以内にしてください。
  各行は「・〇〇」の形式とし、目的や効果の説明、継続・変更・追加などの方針、回数・セット数、詳細な負荷量は出力しないでください。カルテにない部位、運動、治療内容を提案・推測せず、曖昧で分類できない記載は採用しないでください。該当項目がない場合は「・カルテ記載から確認できる実施プログラムなし」とだけ記載してください。
  例：ハムストリングス・腓腹筋・股関節外旋の各ストレッチは「・股関節・下肢筋群のストレッチ」、ブリッジ・クラムシェル・ヒップアブダクションは「・殿筋群・体幹筋群の筋力エクササイズ」のように統合してください。
...
・疼痛、関節可動域、関節弛緩性テスト、筋力、握力、感覚、スペシャルテスト、理学療法評価、ADL、および出力するその他の評価項目を同じ形式に統一してください。測定対象外などで項目を出力しない場合を除き、項目名だけを出力して結果を空欄にしないでください。
・次のレイアウト例に従ってください。例の内容を評価結果として流用せず、与えられたデータを変更・要約せずに記載してください。
  疼痛：
  　安静時0、夜間時0、動作時0
  関節可動域：
  　肩関節屈曲：右120°、左180°
  ...
  　・棘上筋：右MMT4、左MMT5
  　・三角筋：右MMT4、左MMT5
・優先順位が高い問題点を「優先的問題点：」の単独行に続けて、各結果行を全角スペースで字下げし、3つの箇条書きで挙げてください（改善が見込める、かつPT考察から導き出される視点から判断。1項目につき30文字程度で簡潔に）。
・最後に必ず以下の期限をそのまま記載してください：
  【標準算定期限】：{std_deadline_str}
  【リハビリ期限】：{rehab_deadline_str}
【計画書用】
・ここから始まる計画書部分は、上記のカルテ用レイアウト指示の対象外です。以下に指定した従来の見出し・形式を変更しないでください。
・【疼痛について】（20文字以内）
・【筋力について】（20文字以内）
・【感覚異常について】（20文字以内）
